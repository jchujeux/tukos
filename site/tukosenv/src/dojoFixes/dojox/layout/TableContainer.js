/*
 *
 * JCH: - Implement the bug fix suggested at: http://bugs.dojotoolkit.org/ticket/12599 
 *      - Allow horizontal scrolling by sending width to parent when relevant
 *      - Allow different css class for horizontal versus vertical label layout
 */
define(["dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/declare", "dojo/dom-class", "dojo/dom-construct", 
		"dojo/_base/array", "dojo/dom-prop", "dojo/dom-style", "dijit/_WidgetBase", "dijit/layout/_LayoutWidget", "tukos/utils"],
function(kernel, lang, declare, domClass, domConstruct, arrayUtil, domProp, domStyle, _WidgetBase, _LayoutWidget, utils){
			...
			if (child.hidden){
                ...
                if(this.showLabels) {
                    ...
                    if(child.spanLabel) {
                        ...
                        addCustomClass(labelCell, this.orientation == "vert" ? "labelCellv" : "labelCellh"); //JCH to allow different class for horizontal vs vertical
                        var labelProps = {"for": child.get("id")}, label = domConstruct.create("label", labelProps, labelCell), labelCellStyle = child.labelCellStyle ? child.labelCellStyle : this.labelCellStyle;
                ...
                if (this.widgetWidths){
                    if(Number(this.widgetWidths[widgetColumn]) > -1 ||
                        String(this.widgetWidths[widgetColumn]).indexOf("%") > -1) {
                            
                        // Set the width of the label cell with either a pixel or percentage value
                        domStyle.set(childCell, "width",
                            String(this.widgetWidths[widgetColumn]).indexOf("%") < 0
                                ? this.widgetWidths[widgetColumn] + "px" : this.widgetWidths[widgetColumn]);
                    }
                }
                if(colspan > 1) {
                ...
                if (widgetCellStyle){
                    utils.forEach(widgetCellStyle, function(value, att){
                        domStyle.set(childCell, att, att === 'width' && colspan > 1 ? utils.sizeMultiplier(value, colspan) : value);
                    });
                }
