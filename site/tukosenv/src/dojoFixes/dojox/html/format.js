define(["dojo/_base/kernel", "dojox/html/entities", "dojo/_base/array", "dojo/_base/window", "dojo/_base/sniff"], 
  function(lang, Entities, ArrayUtil, Window, has) {
    var defaultInlineTags = {a: true, b: true, strong: true, s: true, strike: true, i: true, u: true, em: true, sup: true, sub: true, span: true, font: true, big: true, cite: true, q: true, small: true, mi: true, mo: true, mn: true, mtext: true};
    var rgxp_fixIEAttrs = /[=]([^"']+?)(\s|>)/g, rgxp_styleMatch = /style=("[^"]*"|'[^']*'|\S*)/gi, rgxp_attrsMatch = /[\w-]+=("[^"]*"|'[^']*'|\S*)/gi;
    ...
    return {
          prettyPrint: function(nodeOrHtml/*String or dom node*/, indentBy /*Integer?*/, maxLineLength /*Integer?*/, map/*Array?*/, /*boolean*/ xhtml, extraInlineTags){
            ...
            var content = [], indentDepth = 0, closeTags = [], iTxt = "\t", textContent = "", inlineStyle = [], i, inlineTags, map = map || htmlEntitiesMapping;
          
            // Check to see if we want to use spaces for indent instead of tab.
            ...
            if (extraInlineTags){
                inlineTags = lang.clone(defaultInlineTags);
                extraInlineTags.forEach(function(tag){
                    inlineTags[tag] = true;
                });
            }else{
            ...
            var isInlineFormat = function(tag){
              // tags we do not wish to break
              return inlineTags[tag] || false;
            };
            ...
            var openTag = function(node){
              ...
              //tag = tag.replace(rgxp_fixIEAttrs,'="$1"$2');// JCH: commented out as screwes-up inline onClick directive for tukos Checkbox and tukos doesn't support IE anyway
