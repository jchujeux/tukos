define(["dojo/_base/lang", "dojo/_base/declare", "dojo/_base/array", "dojo/has",
		"dojox/charting/plot2d/CartesianBase", "dojox/charting/plot2d/_PlotEvents", "dojox/charting/plot2d/common", "dojox/lang/functional", "dojox/lang/utils", "dojox/gfx/fx"],
			...
			for(var i = 0; i < this.series.length; i++){
				...
				for(var seg = 0; seg < rsegments.length; seg++){
					...
					if(indexed){
						lpoly = arr.map(rsegment.rseg, function(v, i){
							return {
								...
								data: v,
								index: rsegment.index//JCH - bug fix
							};
						}, this);
					}else{
					...
					if(indexed && this.opt.interpolate){
						...
						rsegment = rsegments[0]; //JCH - bug fix
					}
					...
					if(this.opt.markers){
						...
						if(events){
							let previousLpolyIndex = -1, lpolyIndexIncrement = 0;//jch begin bug fix
							arr.forEach(frontMarkers, function(s, i){
								if (lpoly[i] && lpoly[i].index){
									if (previousLpolyIndex === lpoly[i].index){
										lpolyIndexIncrement += 1;
									}else{
										lpolyIndexIncrement = 0;
									}
									previousLpolyIndex = lpoly[i].index;
								}//jch end bug fix
								var o = {
									element: "marker",
									index:   (lpoly[i] && (lpoly[i].index + lpolyIndexIncrement)) || (i +  rsegment.index), //jch - bug fix
									run:     run,
									shape:   s,
