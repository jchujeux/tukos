define([
    ...
    "dojo/json",
	"dojo/text!dojoFixes/dojox/form/resources/UploaderFileList.html"
],function(fx, domStyle, domClass, declare, lang, arrayUtil, manager, Base, JSON, template){
...
return declare(/*"dojox.form.uploader.FileList", */Base, {
	...
	// headerFileMdate: String
	//		The label for the file modified date column.
	headerFileMdate:"Last modified date",
	...
	setUploader: function(){
		...
		if(this.uploader){
			...
			if(!(this._fileMdateAvail = {'html5':1}[this.uploader.uploadType])){
				//if uploadType is not html5, file lastModifiedDate is not available
				//hide the modified date header
				this.mdateHeader.style.display="none";
			}
		}else{
			this._upCheckCnt++;
	...
	_onUploaderChange: function(fileArray){
		...
		arrayUtil.forEach(fileArray, function(f, i){
			this._addRow(i+1, this.getFileType(f.name), f.name, f.size, f.mdate);
		}, this)
	},

	_addRow: function(index, type, name, size, mdate){
		...
		if(this._fileMdateAvail){
			c = r.insertCell(-1);
			//domClass.add(c, "dojoxUploaderMdate");
            domStyle.set(c, 'width', '120px');
			c.innerHTML = JSON.stringify(mdate).substring(1,20);
			//c.innerHTML = mdate;
		}
