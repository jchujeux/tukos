define([
	...
	"dojo/query",
	"dijit/focus", // focus.focus(), focus.curNode
	"dijit/_editor/_Plugin",
	"dijit/form/ToggleButton",
	"dijit/registry", // registry.getEnclosingWidget()
	"dojo/i18n!dijit/_editor/nls/commands"
], function(aspect, declare, domClass, domGeometry, domStyle, i18n, keys, lang, on, has, win, winUtils, query,
			focus, _Plugin, ToggleButton, registry){

	// module:
	//		dijit/_editor/plugins/FullScreen

	var FullScreen = declare(_Plugin, {
		// summary:
		...
		_setFullScreen: function(full){
			...
			console.log('just entered _setFullScreen - full = ' + full);
			var ed = this.editor;
			...
			if(full){
				...
				if (ed.pane){
					query(".dgrid-hider-toggle, .tukos-hider-toggle", ed.pane.domNode).forEach(function(node){
						domClass.add(node, "dijitForceStatic");
					});
				}
                ...
                var rawStyle = ed.iframe.parentNode.style || {};
                this._origIframeParentNodeState = {height: rawStyle.height, width: rawStyle.width};
				...
				this.editorOnBlurOld = this.editor._onBlur;
				this.editor._onBlur=function(e){
					//console.log('on blur for the editor');
					e.preventDefault();//JCH e is undefined, so this generates an exception which stops the blur process so that under dgrid, fullscreen is not blurred. Would be better to achieve this result in another way ...
					//e.stopPropagation();
				};

				// Call it once to work around IE glitchiness.  Safe for other browsers too.
/* JCH - sometimes generates a bug where the editor toolbar does not appear in editor. Eliminating it seems not to create an issue (?)
				this._resizeEditor();
...
*/
				...
				this.editor.isFullscreen = false;
				this.editor._onBlur = this.editorOnBlurOld;
				...
				if (ed.pane){
					query(".dgrid-hider-toggle, .tukos-hider-toggle", ed.pane.domNode).forEach(function(node){
						domClass.remove(node, "dijitForceStatic");
					});				
				}
...
*/
                    ...
                    domStyle.set(ed.iframe.parentNode, self._origIframeParentNodeState);
					domStyle.set(ed.iframe, self._origiFrameState);
					delete self._origState;
                    delete self._origIframeParentNodeState;
