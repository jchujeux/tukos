	...
	var ViewSource = declare("dijit._editor.plugins.ViewSource", _Plugin, {
		...
		_showSource: function(source){
			...
			try{
                ...
                var ed = this.editor, sourceArea = this.sourceArea, iframe = ed.iframe;
				if(source){
					...
					ed.onDisplayChanged();
					sourceArea.initialHtml = html = ed.get("value");
					...
					if(this._fsPlugin){
	                 ...
	                 require (["dojoFixes/dojox/html/format"], lang.hitch(this, function(format){
	                    sourceArea.initialValue = sourceArea.value = html;
	                    // Since neither iframe nor textarea have margin, border, or padding, just set sizes equal.
	                    sourceArea.style.height = iframe.style.height;
	                    sourceArea.style.width = iframe.style.width;
	                    ...
	                    domStyle.set(iframe, {position: "absolute", top: 0,visibility: "hidden"});
	                    domStyle.set(sourceArea, {display: "block"});
	                    ...
	                    ed.getValue = sourceArea.getSourceValue;
	                    this._setListenerBefore = aspect.before(ed, "setValue", lang.hitch(this, function(htmlTxt){
	                       sourceArea.initialHtml = htmlTxt;
	                    }), true);
					...
					this._setListenerBefore.remove();
					delete this._setListenerBefore;

					this._setListenerAfter.remove();
					delete this._setListenerAfter;
					...
					domStyle.set(sourceArea, "display", "none");
					domStyle.set(iframe, {
						position: "relative",
						visibility: "visible"
					});
		...
		_createSourceView: function(){
			...
			var ed = this.editor;
			var edPlugins = ed._plugins;
			var sourceArea = this.sourceArea = domConstruct.create("textarea");
			if(this.readOnly){
				domAttr.set(sourceArea, "readOnly", true);
				this._readOnly = true;
			}
			domStyle.set(sourceArea, { padding: "0px", margin: "0px", borderWidth: "0px", borderStyle: "none"});
			domAttr.set(sourceArea, "aria-label", ed.id);

			domConstruct.place(sourceArea, ed.iframe, "before");
			...
			// it from there too.
			this.own(on(sourceArea, "keydown", lang.hitch(this, function(e){
