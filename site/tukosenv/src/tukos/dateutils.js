define(["dojo", "dojox/date/posix"], function(dojo, ISODates){
	...
	return {
        ...
        addDurationString: function(durationString, toDate){
            var durationArray = JSON.parse(durationString);
            if (durationArray[0] && durationArray[1]){
        ...
        parseDate: function(dateString){
           //console.log('dateutils.parseDate - dateString: ' + dateString);
           //return (dateString && dateString.length > 10 && dateString[10] === 'T' ?  this.fromISO(dateString) : dojo.date.locale.parse(dateString, {selector: 'date', datePattern: (dateString.length === 10 ? 'y-M-d' : 'y-M-d H:m:s')}));
