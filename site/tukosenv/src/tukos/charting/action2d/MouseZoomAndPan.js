define(["dojo/_base/declare", "dojo/_base/window", "dojo/_base/array", "dojo/_base/event", "dojo/_base/lang",
	"dojo/_base/connect", "dojo/mouse", "dojo/on", "dojox/charting/action2d/ChartAction", "dojo/sniff", "dojo/dom-prop", 
	"dojo/has!dojo-bidi?dojox/charting/bidi/action2d/ZoomAndPan"],
	function(declare, win, arr, eventUtil, lang, connect, mouse, on, ChartAction, has, domProp, BidiMouseZoomAndPan){
	...
	var sUnit = has("mozilla") ? 100 : 120;// seems for firefox, 120 induces more spurioous scrolling when zooming 
		...
		constructor: function(chart, plot, kwArgs){
			...
			if(this.enableKeyZoom){
				on(this.chart.node, "keydown", lang.hitch(this, this.onKeyDown));
			}
			this._handles = [];
			this.connect();
		},
		...
		onKeyDown: function(event){
			switch(event.key){
				case '+': this._onZoom(1, event); break;
				case '-': this._onZoom(-1, event);break;
			}
		},
		...
		onDoubleClick: function(event){
			...
			var chart = this.chart, axis = chart.getAxis(this.axis);
			var scale = 1 / (2*this.scaleFactor);
			// are we fit?
		...
		_onZoom: function(scroll, event){
			...
			var middle = (event.type == "keydown") ? (start + end) / 2 :
