...
function(declare, BaseClass, modeling, utils, matrices){
	...
	return declare([BaseClass], {
		...
		cSky: function(dimension, problemType, elementType, rheologyType){
		...
		_dHRhsHydraulicspressure: function(group, integrationPoint){
			...
			const correction = (pressure < 0.0 ? 0.0 : 1.0);
			if (pressure < 0.0){
				console.log('pressure is : ' + pressure);
			}
			/*for (let d = 0; d < group.dimension; d++){
				dHRhs.push(correction * gravityDirection[d] * hydraulicConductivity);
			}
			return dHRhs;*/
