...
function(declare, when, ArrayIterator, utils, dutils){
    return declare(null, {
        constructor: function(args){
			...
			//this.collection = this.sessionsStore.filter({mode: 'performed'}).sort([{property: 'startdate'}, {property: 'sessionid'}]);
		...
		getCollection: function(){
			//return this.sessionsStore.filter({mode: 'performed'}).sort([{property: 'startdate'}, {property: 'sessionid'}]);
				...
				var iterator = self.sessionsIterator, idp = collection.idProperty, item, sessionStress, sts, lts, tsb, stsHasChanged, ltsHasChanged, tsbHasChanged, previousItem, daysDifference, previousLts, previousSts,
					...
					if (row === false || (stsHasChanged || ltsHasChanged || tsbHasChanged)){
						...
						while (item = iterator.next()){
							...
							sts = self.exponentialAvg(item.gctrimp100 || 0, previousItem.sts, daysDifference, self.stsDailyDecay);
							grid.updateDirty(item[idp], 'sts', sts);
							lts = self.exponentialAvg(item.gctrimp100 || 0, previousItem.lts, daysDifference, self.ltsDailyDecay);
							grid.updateDirty(item[idp], 'lts', lts);
							grid.updateDirty(item[idp], 'tsb', lts - sts * self.stsRatio);
							previousItem = item;
						}
					}
