define (["dojo/_base/declare", "dojo/_base/lang", "dojo/when", "dojo/dom-attr", "dojo/dom-style", "dojo/dom-class", "dojo/dom-construct", "dojo/string", "tukos/utils", "tukos/expressions", "tukos/PageManager",
		 "tukos/widgets/editor/plugins/_TagEditDialog"], 
    function(declare, lang, when, domAttr, domStyle, dcl, dct, string, utils, expressions, Pmg, TagEditDialog){
    ...
    return declare(TagEditDialog, {
        ...
        openDialog: function(){
            var table = this.table, pane = this.pane/*, paneGetWidget = lang.hitch(pane, pane.getWidget)*/, paneSetWidgets = lang.hitch(pane, pane.setWidgets), tableInfo = this.tableInfo;
       ...
       insertAfter: function(){
        ...
        insertCol: function(after){
            ...
            if (isWorksheet){
            	var rows = tableInfo.rows, trs = tableInfo.trs, cols = tableInfo.cols, rCols = tableInfo.rCols, tds = Array.apply(null, trs[0].children);
            	...
            	for (var r = 1; r < rows; r++){
            		...
            		tds[0].innerHTML = r;
            		for (var c = rCols[r]-1; c > colIndex; c--){
            			expressions.setName(tds[c].children[0], {name: sheetName, col: utils.alphabet(c), row: r}, true);
            		}
            	}
            }
        ...
        deleteRow: function(){
        	...
        	if (tableInfo.rows > 1){
                ...
                if (isWorksheet){
                	var rows = tableInfo.rows, trs = tableInfo.trs, sheetName = table.id, rCols = tableInfo.rCols;
        ...
        deleteCol: function(){
            var tableInfo = this.tableInfo, cols = tableInfo.cols, rCols = tableInfo.rCols;
        	if (cols > 1){
                var colIndex = tableInfo.colIndex, trs = tableInfo.trs, table = this.table, isWorksheet = dcl.contains(table, 'tukosWorksheet');
                tableInfo.trs.forEach(function(row, r){
                    row.deleteCell(Math.min(colIndex, rCols[r]-1));
                });
                tableInfo = this.tableInfo = this.getTableInfo(true);
                table = this.table = this.pane.table = this.tableInfo.tbl;
                if (isWorksheet){
                	...
                	for (var r = 1; r < rows; r++){
                		var tr = trs[r], tds = Array.apply(null, tr.children);
                		for (var c = colIndex; c < rCols[r]; c++){
                			expressions.setName(tds[c].children[0], {name: sheetName, col: utils.alphabet(c), row: r}, true);
                		}
                	}
                }
