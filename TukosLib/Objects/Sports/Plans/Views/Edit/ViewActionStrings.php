...
EOT;
  ...
  protected function exportPaneOnOpenAction(){
      $tukosBackOfficeMailAccountId = Tfk::tukosBackOfficeMailAccountId;
      return <<<EOT
    ...
    return when(this.setWidgets({value: {
        ...
        from: this.valueOf('from') || $tukosBackOfficeMailAccountId,
...
EOT;
  ...
  protected function exportCustomContent($tr){
      $this->view->addToTranslate(['workout', 'sportimage', 'content']);
      return <<<EOT
...
return Pmg.serverDialog({object: 'organizations', view: 'Edit', action: 'GetItem', query: {id: coachOrganization, storeatts: JSON.stringify({cols: ['logo']})}}).then(function(response){
     var logo = response.data.value.logo, html = '', selectedWeeks = self.getWidget('optionalweeks').get('value'), 
        ...
        var colsFormat = {workout: 'string', duration: 'tHHMMSSToHHMM', intensity: 'string', sport: 'string', sportimage: 'image', stress: 'string', content: 'string'}, i = 1, 
            workoutsWidget = form.getWidget('sptworkouts'), workoutsFilter = new workoutsWidget.store.Filter(), weekliesWidget = form.getWidget('weeklies'), weekliesFilter = new weekliesWidget.store.Filter(), intensityColorFlag = this.valueOf('rowintensitycolor'),
            contentCols = ['warmup', 'mainactivity', 'warmdown', 'comments'],
            presentation = self.valueOf('presentation');
        selectedCols.unshift('workout');    
        var numberOfCols = selectedCols.length, rowContent = [], rows = [];
        selectedCols.forEach(function(col){
            rowContent.push({tag: 'th', atts: thAtts, content: workoutsWidget.columns[col] ? workoutsWidget.colDisplayedTitle(col) : Pmg.message(col, 'sptplans')});
        });
        rows.push({tag: 'tr', content: rowContent});
        workoutsWidget.store.filter(workoutsFilter.gte('startdate', firstDay).lte('startdate', lastDay)[mode === 'performed' ? 'eq' : 'ne']('mode', 'performed')).sort('startdate').forEach(function(workout){
            var intensityColorAtt = 'style="background-color: ' + ((intensityColorFlag === 'on' && workout.intensity) ? backgroundColor.map[workout.intensity] : backgroundColor.defaultValue)  + ';"';
            rowContent = [];        
            selectedCols.forEach(function(col){
                var colContent, colTdAtts = tdAtts;            
                switch (col){
                    case 'workout':
                        colContent = utils.capitalize(Pmg.message(dutils.dateToDayName(new Date(workout.startdate)))) + (presentation === 'perworkout' ? '' : (' ' + utils.transform(workout.startdate, 'date', {selector: 'date', datePattern: 'd'})));
                        break;
                    case 'duration':
                        colContent = workout.sport === 'rest' ? '' : workoutsWidget.colDisplayedValue(workout.duration, 'duration');
                        break;
                    case 'content':
                        var activeContent = [];
                        contentCols.forEach(function(col){
                            if (workout[col]){
                                activeContent.push(self.valueOf('prefix' + col) + workout[col]);
                            }
                        });
                        colContent = activeContent.join(self.valueOf('contentseparator'));
                        break;
                    case 'sportimage':
                        colContent = workout.sport ? utils.transform(imagesAtts.imagesDir + imagesAtts.map[workout.sport], 'image') : workout.sportImage;
                        break;
                    case 'coachcomments':
                        colTdAtts = 'style="' + cellBorderStyle + 'background-color: LightGrey; font-style: italic;"';
                    default:
                        colContent = workoutsWidget.colDisplayedValue(workout[col], col);
                }
                ...
                {tag: 'tr', content: {tag: 'td', atts: 'style="background-color: black; color: White; font-size: large; font-weight: bold; ' + cellBorderStyle + '" colspan=' + numberOfCols, content:
                    {tag: 'table', atts: tableAtts, content: {tag: 'tr', content: [
                        ...
                        {tag: 'td', atts: 'style="text-align:center; color: White; font-size: large; font-weight: bold; width: 90%"', content: 
                          '{$tr('trainingplan', 'escapeSQuote')}' + ': ' + form.valueOf('name') + '<br>' + title + ' - ' + (presentation === 'perworkout' 
                            ? ''
                            : ('{$tr('week')} ' + weekOfTheYear + ': {$tr('fromdate')} ' + utils.transform(firstDay, 'date') + ' {$tr('todate')} ' + utils.transform(lastDay, 'date') + ' (')) + 
                         '{$tr('week')} ' + weekOfProgram + ' / ' + this.valueOf('weeksinprogram') + (presentation === 'perworkout' ? '' : ')')}
                    ]}}
                }},
    ...
    selectedWeeks.forEach(function(selectedWeek){
        ...
        switch(selectedWeek){
            case 'plannedthisweek' : 
                html += buildWeeklyTable('', '{$tr('plannedworkouts')}', firstDay, lastDay, weekOfTheYear, weekOfProgram, self.getWidget('plannedcolstoinclude').get('value'));
                break;
            case 'performedthisweek': 
                html += buildWeeklyTable('performed', '{$tr('performedworkouts')}', firstDay, lastDay, weekOfTheYear, weekOfProgram, self.getWidget('performedcolstoinclude').get('value'));
                break;
            case 'plannedlastweek' : 
                html += buildWeeklyTable('', '{$tr('plannedworkouts')}', dutils.formatDate(dojo.date.add(new Date(firstDay), 'week', -1)), dutils.formatDate(dojo.date.add(new Date(lastDay), 'week', -1)),
                            previousWeekOfTheYear, previousWeekOfProgram, self.getWidget('plannedcolstoinclude').get('value'));
                break;
            case 'performedlastweek': 
                html += buildWeeklyTable('performed', '{$tr('performedworkouts')}', dutils.formatDate(dojo.date.add(new Date(firstDay), 'week', -1)), dutils.formatDate(dojo.date.add(new Date(lastDay), 'week', -1)),
...
this.customToSend = lang.getObject('customization.widgetsDescription.stravasync.atts.dialogDescription.paneDescription.widgetsDescription', false, form);
