...
trait ContentExporter {
	...
	function sendContent($query, $atts){
		if (empty($atts['from']) || empty($atts['to'])){
			...
			if ($atts['sendas'] !== 'apppendtobdy'){
				$tmpPdfFileName = $this->buildPdf($atts);
				$mailArgs['attachments'] = [$tmpPdfFileName];
			}
	...
	function fileContent($query, $atts){
			$tmpPdfFileName = $this->buildPdf($atts);
			if ($fileHandle = fopen($tmpPdfFileName, 'r')){
				...
				header("Content-Disposition: attachment; filename=" . basename($tmpPdfFileName));
	...
	function buildPdf($atts, $dir = Tfk::tukosTmpDir){
		$fileName = empty($atts['filename']) ? uniqId() : str_replace(' ', '_',$atts['filename']);
		$dirFileName = $dir . $fileName;
		//$prefix = Tfk::tukosTmpDir . $this->fileName;
		$contentFileName =  $dirFileName . 'body.htm';
		$this->buildHtmlFile($contentFileName, $atts['content']);
		$htmlToPdfOptions = ' ';
		...
		//$htmlToPdfOptions = '--header-spacing 0 --margin-top 130 ';
	...
	function buildHtmlFile($fileName, $content, $htmlHeaderScript = '', $headerDivHeight = null){
		...
		if (!empty($htmlHeaderScript)){
			...
			fwrite($htmlHandle, '<!DOCTYPE HTML><html ><meta charset="utf-8"><head><meta charset="utf-8">' . $htmlHeaderScript . '</head><body style="margin:0; padding: 0;" onload="subst()"><div ' . $styleOptions . '>' . $content . '</div></body></html>');
		}else{
			fwrite($htmlHandle, '<!DOCTYPE HTML><html ><meta charset="utf-8"><head><meta charset="utf-8"></head><body>' . $content . '</body></html>');
		}
		fclose($htmlHandle);
	}
}
