    ...
    public static function getData(&$response, $editModelGet, $contextPathId){
        foreach ($editModelGet->view->subObjects as $widgetName => $subObject){
            if (empty($subObject['noServerGet'])){
                ...
                $subObject['filters'] = array_merge($subObject['filters'], $editModelGet->user->getCustomView($editModelGet->model->objectName, 'edit', $editModelGet->paneMode, ['data', 'filters', $widgetName]));
